...
public class LoaiSachFragment extends Fragment {
    ...
    private int validate(){
        ...
        else if(edTenLS.getText().length()<5||edTenLS.getText().length()>20){
            Toast.makeText(getContext(),"Tên loại sách từ 5 đến 20 kí tự",Toast.LENGTH_SHORT).show();
            check=-1;
        }
        else if(!edTenLS.getText().toString().substring(0,1).matches("^[0-9]")){
            Toast.makeText(getContext(),"Tên loại sách bắt đầu bằng số",Toast.LENGTH_SHORT).show();
            check=-1;
        }
        return check;
    }
